...
public class AudioController {
	...
	private void updateMusic()
	{
		...
		if (count == 1) // Shortcut
		{
			...
			// First calculate the sector in which the dude is (sector in terms of music)
			// Then calculate the dudes position in this sector, if the dude happens to be
			// in the sector border offset zones the transition must be calculated
			float sectorRange = (float)world.getLevel().getHeight() / (float)count;
			...
			if (time % 3000 == 0) {
//				logger.debug("World-Height: " + world.getLevel().getHeight());
